...
t.test(weight_after, weight_before, paired=T, alternative="less") # order matters!
...
# what if
t.test(weight_before, weight_after, paired=T, alternative="greater") # order matters!
...
# if the ratio i snot equal to 1, that means the unequal variance.

res<-t.test(caff, placebo, var.equal=FALSE, paired=FALSE)               
# var.equal=FALSE is the default, so no need to specifically write it.
res
...
# Reject the null and conclude that the means RER are sig diff b/w the two groups.

# 95% CI of the difference: (-17.639668,-6.766628)                     
# We could safely say that the mean RER is sig. lower in the caffeine group (Why?)
