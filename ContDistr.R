...
# P(X=certain number) = 0

# What about dnorm?
dnorm(65, mean = 70, sd = 10) # only give the height of that corresponding point, it's not much useful.
# dnorm calcuates the height of the curve, usually only used for plotting the height.
x_var<-seq(-5,5)
plot(x_var, dnorm(x_var), type = "l")  # this is the density plot of the normal variable.
...
qnorm(c(0.05, 0.025, 0.005), lower.tail = FALSE) # area to the right
# they are same as the following: 
qnorm(c(0.95, 0.975, 0.995), lower.tail = T)
