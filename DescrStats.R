...
#setwd("C:\Users\yaoya\OneDrive\Desktop\P8130_BM\BM_DescriptiveStats")
...
# we calculate birth rate by categorical variable: race.
...
tab1 <- tableby( ~ age + bwt + smoke, data=low_birth_all) # summarize age, bwt, and smoke, this not look good
summary(tab1)
...
# Make 'smoke' a factor to show N (%) # function mutate()
birth_df<-low_birth_all %>% 
          mutate(smoke=factor(smoke, labels=c("No","Yes"))) #smoking was shown as 0 and 1 in the original dataset, we label it as no and yes.
# Start labeling with 0 (increasing order)
...
ggplot(low_birth_all, aes(x = race, y = bwt, fill = smoke)) +
