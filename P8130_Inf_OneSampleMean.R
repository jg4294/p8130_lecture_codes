...
# Calculate their means/var and draw a histogram to visualize the sample means distribution

set.seed(2)
sample_means_exp1 = rep(NA, 1000)

for(i in 1:1000){
  sample_means_exp1[i] = mean(rexp(10,0.3)) # rexp() generate exponential dist.with n=10, lambda=0.3
}
...
# Test if the mean infarct size is different from 25
# X_bar=16, s=10, N=40
...
t.test(low_birth_all$bwt, alternative='two.sided', mu=3000) # mu_null = 3000
